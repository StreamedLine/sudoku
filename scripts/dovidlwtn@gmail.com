(function hideVars() {
    ...
    for (var i = 0; i < 81; i++) {
      (function(j) {
        ...
        domSpot.addEventListener('click', function() {
          ...
          keypad.style.top = spotCoor.top - keypadCoor.height / 2 + spotCoor.height / 2 + 'px';
          keypad.style.left = spotCoor.left - keypadCoor.width / 2 + spotCoor.width / 2 + 'px';

	  keys = keypad.getElementsByTagName('div');
  ...
  document.addEventListener('keypress', function(e) {
    ...
    if (temp == 48)
    	board.board[keyboardInputPosition].dom.innerHTML = ' ';
    else
